...
import com.smallbasket.dozo.databinding.ActivityMyLogsBinding
...
class MyLogsActivity : AppCompatActivity() {
    ...
    private fun setupRecyclerViews() {
        myOrdersAdapter = MyLogsAdapter(emptyList(), isMyOrders = true) { order ->
            navigateToRequestDetail(order)
        }

        myDeliveriesAdapter = MyLogsAdapter(emptyList(), isMyOrders = false) { order ->
    ...
    private fun loadMyOrders() {
        ...
        lifecycleScope.launch {
            ...
            result.onFailure { error ->
                ...
                binding.swipeRefreshOrders.isRefreshing = false
                binding.tvEmptyMessage.visibility = View.VISIBLE
                binding.tvEmptyMessage.text = getString(R.string.error_load_orders_failed)
                binding.rvMyOrders.visibility = View.GONE

                Toast.makeText(
                    this@MyLogsActivity,
                    com.smallbasket.dozo.utils.ErrorUtils.getFriendlyMessage(this@MyLogsActivity, error),
                    Toast.LENGTH_LONG
                ).show()
            }
        }
    ...
    private fun loadMyDeliveries() {
        ...
        lifecycleScope.launch {
            ...
            result.onFailure { error ->
                ...
                binding.swipeRefreshDeliveries.isRefreshing = false
                binding.tvEmptyMessage.visibility = View.VISIBLE
                binding.tvEmptyMessage.text = getString(R.string.error_load_deliveries_failed)
                binding.rvMyDeliveries.visibility = View.GONE
