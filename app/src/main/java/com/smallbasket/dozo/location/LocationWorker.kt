// File: app/src/main/java/com/smallbasket/dozo/location/LocationWorker.kt
...
import com.smallbasket.dozo.models.UpdateGPSLocationRequest
...
class LocationWorker(
    ...
    companion object {
        ...
        private const val INITIALIZATION_DELAY = 500L // Reduced delay
    }
        ...
        if (location != null) {
            ...
            try {
                ...
                val locationData = LocationData.fromLocation(
                    ...
                    activityType = if (isMoving) "MOVING" else "STILL",
                    batteryLevel = repository.getBatteryLevel(applicationContext)
                )
                repository.saveLocation(locationData)

                // CALCULATE WALKING DISTANCE (New Fitness Feature)
                if (isMoving) {
                    val lastLoc = repository.getLastDistanceLocation()
                    if (lastLoc != null) {
                        ...
                        // Only add if distance is reasonable (e.g. > 10m and < 2km for a 15min interval)
                        // This filters out GPS noise and fast vehicle movement
                        if (distance in 10.0..2000.0) {
                            Log.d(TAG, "Fitness: User moved ${distance}m while walking")
                            repository.addWalkingDistance(distance.toDouble())
                        }
                    }
                    repository.setLastDistanceLocation(locationData)
                } else {
                    // If still, reset last distance location to current to avoid jumping
                    repository.setLastDistanceLocation(locationData)
                }
