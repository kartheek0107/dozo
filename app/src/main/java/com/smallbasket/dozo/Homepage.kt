...
import com.smallbasket.dozo.databinding.ActivityHomepageBinding
import com.smallbasket.dozo.location.*
import com.smallbasket.dozo.models.Order
import com.smallbasket.dozo.models.DeliveryRequest
import com.smallbasket.dozo.notifications.NotificationActivity
import com.smallbasket.dozo.repository.OrderRepository
import com.smallbasket.dozo.repository.MapRepository
...
class Homepage : AppCompatActivity() {
    ...
    private val connectivityReceiver = object : BroadcastReceiver() {
        override fun onReceive(context: Context?, intent: Intent?) {
            ...
            Log.d("Homepage", "Connectivity changed: connected=$connected")
            ...
            if (!connected) {
                runOnUiThread {
                    ...
                    // Launch beautiful offline activity
                    val offlineIntent = Intent(context, OfflineActivity::class.java).apply {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        try {
            ...
            try {
                if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.TIRAMISU) {
                    registerReceiver(
                        ...
                        IntentFilter("com.smallbasket.dozo.CONNECTIVITY_CHANGED"),
            ...
            Toast.makeText(this, getString(R.string.error_startup_failed), Toast.LENGTH_LONG).show()
        }
    }
                    ...
                    !permissionCoordinator.hasAllPermissions() -> {
                        permissionCoordinator.requestPermissions(
                            ...
                            if (granted) {
                                ...
                                // Also initialize notifications now that permissions are granted
    ...
    private fun updateNotificationBadge() {
        try {
            val notificationManager = com.smallbasket.dozo.notifications.NotificationManager
                .getInstance(this)
    ...
    private fun initializeLocationTracking() {
        try {
            ...
            Toast.makeText(this, getString(R.string.error_location_setup_failed), Toast.LENGTH_LONG).show()
        }
    }
    ...
    private fun startLocationTracking() {
        ...
        lifecycleScope.launch {
            try {
                ...
                Toast.makeText(this@Homepage, com.smallbasket.dozo.utils.ErrorUtils.getFriendlyMessage(this@Homepage, e), Toast.LENGTH_LONG).show()
            }
        }
    }
    ...
    private fun handleCountError(error: Throwable) {
        runOnUiThread {
            ...
            val message = com.smallbasket.dozo.utils.ErrorUtils.getFriendlyMessage(this@Homepage, error)
            Toast.makeText(this@Homepage, message, Toast.LENGTH_SHORT).show()
        }
    }
    ...
    private fun refreshAllData() {
        ...
        // Check internet on manual refresh
        if (!LocationUtils.checkInternetAndRedirect(this)) {
        ...
        lifecycleScope.launch {
            try {
                // Step 1: Force location update
                if (::locationCoordinator.isInitialized) {
                    Log.d(TAG, "Forcing location update during manual refresh")
                    locationCoordinator.forceUpdateLocation()
                }

                // Step 2: Update connectivity
                if (::connectivityManager.isInitialized) {
                    connectivityManager.forceUpdate()
                ...
                Toast.makeText(this@Homepage, com.smallbasket.dozo.utils.ErrorUtils.getFriendlyMessage(this@Homepage, e), Toast.LENGTH_SHORT).show()
            } finally {
                refreshLayout.isRefreshing = false
            }
        }
        ...
        lifecycleScope.launch {
            try {
                ...
                result.onFailure { error ->
                    ...
                    runOnUiThread {
                        loadingText.text = com.smallbasket.dozo.utils.ErrorUtils.getFriendlyMessage(this@Homepage, error)
                    }
                }
                ...
                runOnUiThread {
                    loadingText.text = com.smallbasket.dozo.utils.ErrorUtils.getFriendlyMessage(this@Homepage, e)
                }
            }
        }
    ...
    /**
     * Set up the fitness dial click listener
     */
    private fun setupFitnessDial() {
        binding.fitnessDialContainer.setOnClickListener {
            performSelectionHaptic(it)
            showFitnessDetailsDialog()
        }
    }

    /**
     * Show detailed fitness stats in a popup
     */
    @SuppressLint("SetTextI18n")
    private fun showFitnessDetailsDialog() {
        try {
            val dialogView = LayoutInflater.from(this).inflate(R.layout.dialog_fitness_details, null)
            ...
            val distanceMeters = repository.getDailyWalkingDistance()
            val distanceKm = distanceMeters / 1000.0

            // Format stats
            val distanceStr = String.format(java.util.Locale.US, "%.2f km", distanceKm)
            val steps = (distanceMeters * 1.31).toInt() // Rough estimate: 1.31 steps per meter
            val co2Saved = distanceKm * 0.192 // Avg 192g CO2 per km for a car
            val calories = distanceKm * 60 // Avg 60 kcal per km walking

            dialogView.findViewById<TextView>(R.id.tvPopupDistance).text = distanceStr
            dialogView.findViewById<TextView>(R.id.tvPopupSteps).text = "~%,d".format(steps)
            dialogView.findViewById<TextView>(R.id.tvPopupImpact).text = String.format(java.util.Locale.US, "%.2f kg", co2Saved)
            dialogView.findViewById<TextView>(R.id.tvPopupCalories).text = "${calories.toInt()} kcal"

            dialogView.findViewById<Button>(R.id.btnFitnessClose).setOnClickListener {
                performLightHaptic(it)
                dialog.dismiss()
            }
            ...
            Log.e(TAG, "Error showing fitness details", e)
        }
    }

    /**
     * Update the fitness milestone card with daily walking distance
     */
    @SuppressLint("SetTextI18n")
    private fun updateFitnessStats() {
        try {
            val repository = LocationRepository.getInstance(this)
            val distanceMeters = repository.getDailyWalkingDistance()
            val distanceKm = distanceMeters / 1000.0
            ...
            val distanceStr = String.format(java.util.Locale.US, "%.1f", distanceKm)

            binding.tvFitnessDialDistance.text = distanceStr

            // Update circular progress (target 5km = 5000m)
            binding.fitnessProgressBar.progress = distanceMeters.toInt().coerceAtMost(5000)
