...
import com.smallbasket.dozo.databinding.ActivityOrderBinding
...
class OrderActivity : AppCompatActivity() {
    ...
    private fun setupListeners() {
        binding.btnPlaceNow.setOnClickListener {
            ...
            if (item.isEmpty()) {
                performMediumHaptic()
                Toast.makeText(this, getString(R.string.error_item_required), Toast.LENGTH_SHORT).show()
                return@setOnClickListener
            }

            if (pickup.isEmpty()) {
                binding.pickupError.text = getString(R.string.error_pickup_required)
                binding.pickupError.visibility = View.VISIBLE
                hasError = true
            }

            if (drop.isEmpty()) {
                binding.dropError.text = getString(R.string.error_drop_required)
                binding.dropError.visibility = View.VISIBLE
                hasError = true
            }

            if (pickupArea == "Select Location Area") {
                Toast.makeText(this, getString(R.string.error_pickup_area_required), Toast.LENGTH_SHORT).show()
                return@setOnClickListener
            }

            if (dropArea == "Select Location Area") {
                Toast.makeText(this, getString(R.string.error_drop_area_required), Toast.LENGTH_SHORT).show()
                return@setOnClickListener
            }
